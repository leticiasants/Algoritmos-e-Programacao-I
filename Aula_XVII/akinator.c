...
typedef struct tno{
    char * string; // Aponta tanto para a pergunta quanto para o personagem
    struct tno * esq;
    struct tno * dir;
} no;
...
void insere_personagem(no * arvore, char * nome_personagem, int * respostas, int num_perguntas){
    ...
    if(respostas[num_perguntas-1] == 1) atual->esq = criar_no(nome_personagem);
